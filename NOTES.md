## 2026-06-09 v3.1 v3.0 UI cascade 원복 3.1.16 조건부 승인

- 최종검수는 조건부 승인으로 정리했습니다. 2026-06-09 00:50 KST visual QA summary/json과 대표 PNG, QA 추가 DOM/CSS 63개 조합 실측 보고, HTTP `200`, 전체 unittest 47개 통과를 근거로 Git 자동화 단계 진행이 가능합니다.
- 운영상 남은 확인 사항: service worker cache `workout-pwa-v3.1.16` 반영, axe 잔여 항목 `aria-prohibited-attr`, `color-contrast`, `label`, `meta-viewport`, `scrollable-region-focusable`, 공식 visual QA PNG 대상 확대 후보입니다.
- 수동 확인 권장: 운영 반영 전 실제 브라우저에서 기본 QA 계정 `test / 1234`로 `/app`, `/summaries/weekly`, `/meals/weekly`, `/more`, `/tools/plate-calculator`의 header, tab, arrow, card surface를 한 번 더 확인합니다.
## 2026-06-07 v3.1 v3.0 UI cascade 원복 3.1.12 조건부 승인

- 최종검수는 조건부 승인으로 정리했습니다. 2026-06-06 23:54~23:59 KST visual QA summary/json과 대표 PNG, 추가 DOM/CSS 133개 조합 실측, HTTP `200`, 지정 unittest 32개 통과를 근거로 Git 자동화 단계 진행이 가능합니다.
- 운영상 남은 확인 사항: service worker cache `workout-pwa-v3.1.12` 반영, axe 잔여 항목 44건, 공식 visual QA PNG 대상 확대 후보입니다.
- 수동 확인 권장: 운영 반영 전 실제 브라우저에서 기본 QA 계정 `test / 1234`로 `/app`, `/meals/weekly`, `/meals/monthly`, `/summaries/exercises`의 header, arrow, radius를 한 번 더 확인합니다.
...
- 수동 확인 권장: 실제 사용 데이터가 많은 계정에서 `/app?mode=meal`의 식단 상태 카드 상태별 badge와 action link를 한 번 더 확인합니다.
- QA 도구 개선 후보: `visual-qa.mjs`의 `today` 경로에 `/app?mode=meal` 직접 PNG 캡처를 추가하면 최종검수 근거가 더 명확해집니다.

## 2026-06-05 식단 입력 CTA 및 끼니 선택 노출 보정 조건부 승인

- 최종검수는 조건부 승인으로 정리했습니다. 2026-06-05 17:24 KST visual QA summary/json과 대표 PNG, `/app?mode=meal` DOM/CSS/동작 검증, HTTP `200`, 컴파일, 지정 unittest 21개 통과를 근거로 Git 자동화 단계 진행이 가능합니다.
- 운영상 남은 확인 사항: `aria-prohibited-attr`, `color-contrast`, `label`, `meta-viewport`, `scrollable-region-focusable` axe 경고는 별도 접근성 개선 작업으로 분리합니다.
- 수동 확인 권장: 실제 사용 데이터가 많은 계정에서 `/app?mode=meal`의 자주 먹는 음식 후보와 저장 후 오늘 식단 기록 반영을 한 번 더 확인합니다.
- QA 도구 개선 후보: `visual-qa.mjs`의 `today` 경로에 `/app?mode=meal` 직접 PNG 캡처를 추가하면 최종검수 근거가 더 명확해집니다.
...
## 2026-06-05 식단 입력 전용 UI 재기획 회귀 보정 조건부 승인

- 최종검수는 조건부 승인으로 정리했습니다. 2026-06-05 14:19 KST visual QA summary/json과 대표 PNG, HTTP `200`, 컴파일, 지정 unittest 21개 통과를 근거로 Git 자동화 단계 진행이 가능합니다.
- 운영상 남은 확인 사항: `aria-prohibited-attr`, `color-contrast`, `label`, `meta-viewport`, `scrollable-region-focusable` axe 경고는 별도 접근성 개선 작업으로 분리합니다.
- 수동 확인 권장: 실제 사용 데이터가 많은 계정에서 `/app?mode=meal`의 자주 먹는 음식 후보, 조합 탭, 음식 카드 추가/삭제, 저장 후 오늘 식단 기록 badge 흐름을 한 번 더 확인합니다.
## 2026-06-05 식단 입력 운동 입력 패턴 재구성 조건부 승인

- 최종검수는 조건부 승인으로 정리했습니다. 2026-06-05 12:39 KST visual QA summary/json과 대표 PNG, HTTP `200`, 컴파일, 지정 unittest 21개 통과를 근거로 Git 자동화 단계 진행이 가능합니다.
- 운영상 남은 확인 사항: `aria-prohibited-attr`, `color-contrast`, `label`, `meta-viewport`, `scrollable-region-focusable` axe 경고는 별도 접근성 개선 작업으로 분리합니다.
- 수동 확인 권장: 실제 사용 데이터가 많은 계정에서 `/app?mode=meal`의 빠른 음식 후보, 조합 탭, 음식 빌더 preset, 저장 badge 흐름을 한 번 더 확인합니다.
- Git 자동화 전 `git status --short`에 남은 `tools/watch-agent-supervisor.ps1` 수정 포함 여부와 로컬 자동화 산출물 포함 범위를 확인해야 합니다.

## 2026-06-05 식단 구분 선택 노출 회귀 보정 조건부 승인

- 최종검수는 조건부 승인으로 정리했습니다. 최신 visual QA summary/json과 대표 PNG, HTTP `200`, 컴파일, 지정 unittest 21개 통과를 근거로 Git 자동화 단계 진행이 가능합니다.
- 운영상 남은 확인 사항: `aria-prohibited-attr`, `color-contrast`, `label`, `meta-viewport`, `scrollable-region-focusable` axe 경고는 별도 접근성 개선 작업으로 분리합니다.
- 수동 확인 권장: 실제 사용 데이터가 많은 계정에서 `/app?mode=meal`의 끼니 선택, 빠른 음식 목록, 오늘 식단 기록 badge 흐름을 한 번 더 확인합니다.
