...
- `7954ded` 개인화 다음 운동 인사이트 추가
- `2f44079` 2.7 QA 리포트 기준 정리
- `dbe1b37` 주요 화면 성능 예산 확대
- `bafb294` 세트 입력 반복 저장 개선
- `676047e` 운동명 병합 기능 추가
...
## 2026-05-29 v2.6.4 설정/관리자 역할 분리

- 사용자 설정 탭은 비밀번호 변경만 남기고, 앱 기본값/데이터 관리/QA/리마인더/위험 작업은 관리자 대시보드 운영 관리 영역으로 이동했습니다.
- 설정 비밀번호 재설정은 관리자 페이지에서만 처리하도록 옮겼습니다.
- 운영성 POST 작업은 관리자 전용 endpoint로 묶고, 관리자 화면에서 실행하면 관리자 대시보드로 돌아오도록 정리했습니다.

## 2026-05-29 v2.6.5 분석 페이지 속도 개선

- 주간 분석은 중복 리포트/목표 조회 제거로 DB 쿼리 수가 약 67개에서 34개로 줄었습니다.
- 월간 분석은 중복 리포트/목표 조회와 목표값 일괄 조회 적용으로 DB 쿼리 수가 약 92개에서 57개로 줄었습니다.
- 앱 타임존 객체와 Jinja 템플릿 바이트코드 캐시를 적용해 반복 렌더링 비용을 줄였습니다.

## 2026-05-29 v2.6.6 성능/모바일/관리자 정리

- 앱 기본값 조회를 요청당 1회 일괄 조회로 바꿔 오늘/운동/식단/분석 화면의 반복 설정 쿼리를 줄였습니다.
- 목표 진행률 계산의 주간/월간 집계 쿼리를 합쳐 운동 탭과 분석 페이지의 추가 병목을 낮췄습니다.
- `static/js/app.js`에서 세트 빌더 로직을 `static/js/set_builder.js`로 분리하고 layout/service worker precache에 반영했습니다.
- 관리자 운영 관리 영역에 바로가기 내비게이션과 섹션 앵커를 추가해 데이터 관리, QA, 리마인더, 위험 작업을 찾기 쉽게 정리했습니다.
- 430px 이하 모바일에서 상단 메뉴를 3열 그리드로 바꾸고 운동 빠른 메뉴 sticky를 해제해 Galaxy Fold 계열에서 메뉴가 겹치지 않게 했습니다.
- 검증 기준: 핵심 렌더링, 서비스워커 자산, 관리자 계정 플로우, 성능 쿼리 회귀, JS 문법, Ruff, release check를 통과해야 합니다.

## 2026-05-29 v2.6.7 추가 성능/분리/QA 강화

- 관리자 운영 상태 조회에 요청 단위 캐시를 적용하고 운영 partial에서 쓰지 않는 계정 조회를 제거해 `/admin` 쿼리를 35개에서 23개로 줄였습니다.
- 운동탭 다음 세트 추천을 운동별 반복 조회에서 배치 조회로 바꿔 `/app?mode=workout` 쿼리를 80개에서 73개로 줄였습니다.
- `summaries/summary.html`을 주간, 월간, 기간 기록, 부위별 분석 partial로 분리해 513줄 템플릿을 21줄 진입 파일로 정리했습니다.
- 기록 점검 화면에 정리 우선순위와 빠른 처리 액션을 추가해 중복 운동명, 이상 입력, 누락일을 한 번에 확인할 수 있게 했습니다.
- 모바일 Fold CSS 계약 테스트와 세트 빌더 정적 자산 렌더링 검증을 추가했습니다.

## 2026-05-29 v2.6.8 운동명 정리와 입력 단축 개선

- 기록 점검 화면에서 중복 운동명을 첫 번째 이름으로 병합할 수 있게 했습니다. 병합 시 세트 기록, PR, 루틴, 운동 계획, 운동 설정, 운동 메모까지 같은 대상 운동명으로 이동합니다.
- 오늘 운동의 세트 빌더에 `첫 세트 복제` 단축 버튼을 추가해 목표 세트 수만큼 같은 무게/횟수/세트 타입을 빠르게 채울 수 있게 했습니다.
- 오늘 기록 카드의 빠른 세트 추가에 `추가 세트` 수량을 넣어 같은 세트를 한 번에 최대 10세트까지 저장할 수 있게 했습니다.
- 주요 화면 쿼리 수 회귀 예산을 기록, 더보기, 장소, 일간/PR/장비/운동 분석, 주간/월간 식단까지 확대했습니다.
- 검증 기준: 운동명 병합 단위 테스트, 반복 저장 단위 테스트, Fold UI 회귀 테스트, JS 문법 검사, Ruff, 주요 화면 쿼리 수 테스트를 통과해야 합니다.

## 2026-05-29 v2.7.0 실사용 안정화 릴리스

- 2.6.x에서 진행한 운동명 병합, 세트 반복 저장, 성능 예산 확대, 관리자/QA 정리를 실사용 안정화 버전으로 묶었습니다.
- SW QA 리포트의 오래된 `2.0 준비 상태` 표기를 `2.7 릴리스 준비 상태`로 바꿔 현재 릴리스 기준과 맞췄습니다.
- 릴리스 기준은 운동 입력, 기록, 분석, 식단, 관리자, 인증, 정적 자산, 쿼리 예산 전체 QA입니다.
- `VERSION`, manifest, service worker cache를 `2.7.0`으로 맞췄습니다.
- 검증 기준: Ruff, compileall, 전체 JS syntax check, 전체 unittest, release check를 모두 통과해야 합니다.

## 2026-05-29 v2.8.0 개인화 인사이트 릴리스

- 실행 인사이트에 `개인화 다음 운동` 카드를 추가했습니다.
- 최근 21일 부위별 세트 수, 마지막 운동일, 후보 운동, 최근 장소를 기준으로 다음 운동 초점과 목표 세트를 계산합니다.
- 개인화 계산 로직은 `health_tracker/services/personalization.py`로 분리했습니다.
...
## 2026-05-29 v2.8.2 회색톤 내부 요소 정렬

- 배경만 밝아지고 버튼/카드 내부가 어두운 문제를 해결하기 위해 버튼, 카드, 폼, 리스트, 배지, 로그인 탭 내부색을 공통 override로 정리했습니다.
- 주요 텍스트는 검은색 계열로 고정하고, 보조 문구는 진한 회색 계열로 맞췄습니다.
- primary/danger 버튼은 진한 배경 대신 연한 파랑/빨강 배경과 검은색 글자로 맞췄습니다.
- `VERSION`, manifest, service worker cache를 `2.8.2`로 갱신했습니다.

## 2026-05-29 v2.8.3 로그인 Bad Request 수정

- 서버 재시작 뒤 브라우저에 남아 있던 오래된 CSRF 토큰으로 로그인하면 400 Bad Request가 뜨던 문제를 수정했습니다.
- 로그인/회원가입 같은 공개 POST는 오래된 공개 토큰을 새 토큰으로 교체하고 인증 흐름을 계속 진행합니다.
- 인증이 필요한 POST의 CSRF 차단은 유지합니다.
- 회귀 테스트에 서버 재시작 후 오래된 공개 CSRF 토큰 로그인 케이스를 추가했습니다.

## 2026-05-29 v2.8.4 회색톤 UI 보강 및 식단 저장 안정화

- 기존 CSS에 남아 있던 검은색 내부 배경을 강제 회색톤 override로 추가 정리했습니다.
- 버튼, 카드, 패널, 리스트, 행, 입력칸 내부색을 밝은 회색/흰색으로 통일하고 글자색은 검은색 계열로 맞췄습니다.
- 식단 입력/수정/삭제/복사/즐겨찾기 관련 폼에 서버 렌더링 CSRF 토큰을 직접 추가했습니다.
- 식단 페이지 렌더링에 CSRF 토큰이 포함되는 회귀 검증을 추가했습니다.
