# Changelog
## 2026-07-26 코칭 문구 다양화, PR 강도별 축하 메시지 3.1.26

- "SW가 기계적으로 느껴진다"는 의견을 반영해, 매일 토씨 하나 안 바뀌던 코칭 문구(오늘 컨디션 안내, 회복 추천)에 상태별 2~3개 표현 변형을 두고 무작위로 고르도록 바꿨습니다. 조언의 의미(무게/볼륨 유지·도전 등)는 그대로 유지됩니다.
- PR 축하 토스트를 갱신 폭에 따라 다르게 표시하도록 바꿨습니다: 이 운동 첫 기록("🌱 첫 기록을 남겼습니다!"), 소폭 갱신("🎉 신기록 달성!"), 5~15% 갱신("🔥 멋진 신기록!"), 15% 이상 큰 갱신("🚀 놀라운 신기록!"). `record_pr_events`가 갱신 폭 정보를 반환하도록 바꾸고 `classify_pr_tier`로 등급을 매겼습니다.
- 구현 중 발견한 JS 버그 수정: `PR_TIER_MESSAGES` 상수를 참조하는 함수가 그 상수 선언보다 먼저 호출되면서(같은 실행 순서 문제가 지난 버전 스트릭 토글에서도 있었음) 축하 토스트와 이후 초기화 코드 전체가 조용히 실패하던 문제를 상수 선언 순서를 정리해 해결했습니다.
- `VERSION`/`sw.js`/`manifest`를 `3.1.26`으로 갱신했습니다.
- 전체 unittest 63개(신규 8개), 로컬 서버에서 첫 기록/큰 폭 갱신 두 등급의 실제 토스트 문구를 직접 확인했습니다.

## 2026-07-26 오늘 운동 목록 숨기기 토글 추가 3.1.25

- 세트를 계속 추가할수록 "📋 오늘 운동" 목록이 길어져 화면을 많이 차지한다는 피드백을 반영해, 목록 전체를 숨기고 필요할 때만 펼쳐볼 수 있는 "숨기기/보기" 토글을 추가했습니다.
- 숨김 상태는 `localStorage`에 저장되어 세트를 저장(페이지 재로딩)해도 유지되므로, 운동 중 계속 숨겨둔 채로 세트만 추가할 수 있습니다.
- 목록이 접혀 있어도 헤더에 몇 개 운동이 있는지("N개") 표시되어 완전히 정보가 사라지지 않습니다.
- 운동 기록이 없는 날은 토글 버튼이 표시되지 않습니다(숨길 게 없으므로).
- 구현 중 발견한 JS 버그 수정: `const` 선언보다 먼저 해당 값을 참조하는 초기화 함수를 호출해서 토글이 전혀 동작하지 않던 문제를 선언 순서를 바꿔 해결했습니다.
- `VERSION`/`sw.js`/`manifest`를 `3.1.25`로 갱신했습니다.
- 전체 unittest 57개 통과, 로컬 서버에서 토글 클릭 시 상태 변경과 페이지 새로고침 후에도 숨김 상태가 유지되는지 직접 확인했습니다.

## 2026-07-26 세트 빌더 카드 그라디언트 톤 적용 3.1.24

- "오늘 상태" 카드에 적용한 스포츠 그라디언트 톤을 세트 빌더 입력 카드에도 확장했습니다.
- 세트 번호 뱃지("1세트" 등)를 블루 그라디언트 알약으로, 삭제(X) 버튼을 레드 그라디언트로 바꿔 시각적으로 더 뚜렷하게 구분했습니다. 복사 버튼은 블루 틴트 배경으로, 입력 필드는 포커스 시 파란 글로우가 보이도록 다듬었습니다.
- 세트 입력 카드와 세트 수 툴바 상단에 얇은 하이라이트 라인을 넣어 카드 재질감을 통일했습니다.
- 적용 중 발견한 버그 수정: 세트 번호 뱃지에 새 그라디언트를 넣었는데도 기존 `.set-entry-row > strong` 규칙(더 높은 우선순위)에 가려 반영되지 않던 문제를 선택자를 좁혀 해결했습니다.
- `VERSION`/`sw.js`/`manifest`를 `3.1.24`로 갱신했습니다.
- 전체 unittest 57개 통과, 로컬 서버에서 세트 번호/복사/삭제 버튼의 실제 계산된 스타일 값을 직접 확인했습니다.

## 2026-07-26 오늘 상태 카드 그라디언트 스탯 타일 적용 3.1.23

- UI 고급화 방향 3가지(차분한 프리미엄/다크 글래스/스포츠 그라디언트) 시안을 만들어 검토한 결과 "스포츠 그라디언트" 방향을 채택했습니다.
- "오늘 상태" 카드의 연속 기록/오늘 신기록/오늘 볼륨을 색이 뚜렷한 그라디언트 스탯 타일 3개로 재구성했습니다(스트릭=주황, PR=골드, 볼륨=블루). 아직 신기록이나 스트릭이 없는 날은 타일을 무채색으로 낮춰 표시해 실제 성취와 구분했습니다.
- 운동 기록 카드 헤더의 PR 뱃지와 펼친 상세의 PR 뱃지를 골드 그라디언트 알약 형태로 통일했습니다.
- 시각 검증 중 발견한 버그 수정: 연속 기록 스트릭이 "보고 있는 날짜" 기준으로 계산되고 있어서, 과거 날짜를 조회하면 그 이후에 기록된 활동까지 끌어와 실제로는 기록이 없는 날에도 스트릭이 표시되는 문제가 있었습니다. 실제 오늘 날짜 기준으로 계산하도록 고쳐서 어떤 날짜를 보든 항상 같은(실제) 스트릭 값이 표시되도록 했습니다.
- `VERSION`/`sw.js`/`manifest`를 `3.1.23`으로 갱신했습니다.
- 전체 unittest 57개(회귀 테스트 1개 추가), 로컬 서버에서 실제 데이터로 그라디언트 타일 색상·비활성 상태·스트릭 날짜 일관성을 직접 확인했습니다.

## 2026-07-26 연속 기록 스트릭, PR 축하 강화 3.1.22

- 소스 정리: 루트/instance에 쌓인 stray 서버 로그(gitignore 대상, ~8MB) 삭제, 5/30일자 1회성 QA 메모 2개 제거(ui_rebuild.css 복원으로 이미 해결됨), `app_accounts/app_settings/app_database/app_data.py`를 다른 파사드와 동일하게 `_facade` 접미사로 통일했습니다.
- 연속 기록 스트릭: 운동 세트 또는 식단 기록이 있는 날을 기준으로 며칠 연속 기록했는지 "오늘 상태" 카드에 표시합니다(`🔥 오늘 기록 중 · N일째`). 하루를 걸러도 바로 끊김 처리하지 않고 "어제까지 기록 · 오늘 이어가면 계속됩니다"로 유예를 주고, 이틀 이상 비면 0으로 초기화합니다.
- PR 표시 개선: 신기록이 나와도 카드를 펼쳐야만 보이던 문제를 보완해, 접힌 상태에서도 보이는 헤더 줄에 `🏆 PR N` 뱃지를 색상과 함께 표시했습니다. 세트 저장 직후에는 "🎉 신기록 달성!" 토스트가 한 번 뜨도록 추가했습니다.
- 2026년 운동앱 트렌드를 리서치해 로드맵 후보(다음 세트 자동 제안, 스트릭, PR 축하)를 검토한 결과 다음 세트 자동 제안은 이미 `progressive_overload.py`로 구현되어 있어 제외하고, 진짜 없던 스트릭과 PR 가시성만 신규로 추가했습니다.
- `VERSION`/`sw.js`/`manifest`를 `3.1.22`로 갱신했습니다.
- 전체 unittest 56개, 로컬 서버에서 실제 세트 저장 시 스트릭/PR 뱃지가 렌더링되는지 브라우저로 직접 확인했습니다.
...
- "집중 모드"/"집중 해제" 버튼을 운동/식단/요약 그리드 안에 끼워 넣지 않고 자기 줄 전체를 차지하도록(`grid-column: 1 / -1`) 바꿔 눈에 잘 띄게 했습니다.
- "운동 입력" 섹션에 파란 테두리/배경 강조를 추가하고, "운동 추가" 버튼을 `btn-small`에서 `btn-primary`로 키우고 `+` 표시를 붙였습니다(집중 모드 여부와 무관하게 항상 적용).
- `VERSION`/`sw.js`/`manifest`를 `3.1.20`으로 갱신했습니다.
- 전체 unittest 50개, 집중 모드/일반 모드 모바일 375px 콘솔 에러·가로 스크롤 없음, 버튼 grid-column/색상 값 확인을 통과했습니다.

## 2026-07-24 운동 집중 모드 개선 3.1.19

- 실사용 피드백: 운동 기록 입력 폼이 화면 아래쪽에 있어 찾기 어렵고, 기존 "집중 모드" 토글도 눈에 잘 안 띈다는 의견을 반영했습니다.
- "집중 모드" 버튼에 아이콘(🎯)과 강조 색상을 추가해 더 잘 보이게 했습니다.
- 집중 모드 진입 시 운동 시간(타이머)을 화면 상단에 고정(sticky)하고, 오늘 운동 코치 추천·오늘 할 일 통계·운동 룰셋·루틴 진행 섹션을 숨겨서 세트 입력 폼과 오늘 기록 리스트가 타이머 바로 아래에 오도록 정리했습니다.
- `VERSION`/`sw.js`/`manifest`를 `3.1.19`로 갱신했습니다.
- 전체 unittest 50개, 집중 모드 스티키 동작(스크롤 후에도 타이머가 상단에 고정되는지) 직접 확인, 모바일 375px 가로 스크롤 없음을 확인했습니다.

## 2026-07-24 접근성/데이터 정합성 수정, 리마인더·체형비교 기능 추가, 부위 세분화 3.1.18

- 접근성: `meta viewport`의 `maximum-scale=1.0` 제거, `records/search.html` 검색창에 `aria-label` 추가, 색약/저시력 대비를 위해 `--dim`/`--empty` 텍스트 색상 토큰을 WCAG AA(4.5:1) 기준으로 보정했습니다.
- 데이터 정합성: `GET /app`(오늘 화면)도 `/plans/weekly`와 같은 패턴으로 조회만 해도 빈 `workout_sessions`가 생기던 문제를 읽기 전용 조회로 바꿔 제거했습니다. 이 회귀를 막는 `tests/test_data_integrity.py`를 추가했습니다.
- 리마인더: 관리자 페이지에만 있던 리마인더 설정을 사용자 `/settings` 화면으로 옮기고, 앱이 열려있는 동안 1분 간격으로 알림 조건을 재확인하도록 개선했습니다(로컬 날짜 계산 버그도 함께 수정).
- 체형 변화 비교: 진행 사진 두 장을 슬라이더로 비교하는 화면(`/body/photos`)을 추가했습니다.
- 부위 분류: `팔`을 `팔(이두)`/`팔(삼두)`로 나눴습니다. 운동 룰셋, 회복 상태, 부위 밸런스, 주간 추천 루틴 등 관련 로직을 모두 갱신했습니다. 기존에 저장된 `팔` 기록은 과거 표기 그대로 유지됩니다.
- `VERSION`/`sw.js`/`manifest`를 `3.1.18`로 갱신하고 서비스워커 프리캐시 목록에 새 JS 파일을 추가했습니다.
- `tools/check_release.py`, 전체 unittest 50개, 모바일(360px)/데스크톱 주요 화면 콘솔 에러·overflow 확인을 통과했습니다.
...
- `VERSION`, `static/sw.js`, `static/manifest.webmanifest`를 `3.1.16`과 `workout-pwa-v3.1.16` 기준으로 확인해 이전 CSS/service worker cache 재사용 경로를 줄였습니다.
- 최종검수에서 2026-06-09 00:50 KST Playwright 화면검수 산출물, 추가 DOM/CSS 63개 조합 실측 보고, 대표 PNG 확인, 로컬 HTTP `200`, 전체 unittest 47개 통과를 확인해 조건부 승인했습니다. axe 잔여 항목은 후속 접근성 개선으로 분리합니다.
...
- 사용자 화면에서 `오늘 연료 상태`, `연료 상태`, `연료 코치` 문구가 보이지 않도록 회귀 확인을 추가했습니다.
- 기존 `fuel_coach` 내부 selector/helper는 유지하면서 visible text와 테스트 기대값만 사용자 기준 용어로 정리했습니다.
- 최종검수에서 2026-06-05 18:21 KST Playwright 화면검수 산출물, `/app?mode=meal` DOM/CSS 검증, 로컬 HTTP `200`, 컴파일, 지정 unittest 21개 통과를 확인해 조건부 승인했습니다. 잔여 axe 경고는 별도 접근성 개선으로 분리합니다.

## 2026-06-05 식단 입력 CTA 및 끼니 선택 노출 보정

- `/app?mode=meal`의 식단 입력 접힘 CTA를 `입력`으로 정리하고, 접힘 상태에서 끼니 선택 UI가 노출되지 않도록 보정했습니다.
- 펼침 상태에서는 끼니 선택이 첫 조작으로 보이고 음식 카드, 자주 먹는 음식 후보, 저장/취소 흐름이 이어지도록 확인했습니다.
- 음식 카드 head 2열, `개/g/kcal` 3열 fields, 42px 삭제 버튼, 후보 클릭 입력 반영과 취소 후 접힘 복귀를 QA와 최종검수에서 확인했습니다.
- 최종검수에서 2026-06-05 17:24 KST Playwright 화면검수 산출물, `/app?mode=meal` DOM/CSS/동작 검증, 로컬 HTTP `200`, 컴파일, 지정 unittest 21개 통과를 확인해 조건부 승인했습니다. 잔여 axe 경고는 별도 접근성 개선으로 분리합니다.

## 2026-06-05 식단 기록 압축 UI 재기획

- `/app?mode=meal`의 기본 흐름을 `오늘 식단 -> 식단 입력 -> 식단 도구 -> 보조 요약` 순서로 재정렬했습니다.
- 식단 조합, 지난 식단 복사, 즐겨찾기 음식을 `식단 도구` tab과 compact row로 압축하고 반복 full-width action 버튼을 제거했습니다.
- 목표 달성률과 체성분은 기본 접힘 요약으로 낮춰 모바일 세로 길이를 줄였습니다.
- 최종검수에서 2026-06-05 16:22 KST Playwright 화면검수 산출물, `/app?mode=meal` DOM/CSS/동작 검증, 로컬 HTTP `200`, 컴파일, 지정 unittest 21개 통과를 확인해 조건부 승인했습니다. 잔여 axe 경고는 별도 접근성 개선으로 분리합니다.

## 2026-06-05 식단 입력 전용 UI 재기획 회귀 보정

- `/app?mode=meal`의 식단 입력에서 `입력 기준` select와 관련 문구를 제거하고, 음식명·개·g·kcal를 한 음식 카드 안에서 바로 입력하는 구조를 확정했습니다.
- `QA-2026-06-05-01` 음식 카드 내부 CSS/레이아웃 회귀를 보정해 360/390/656/768/1280 viewport에서 음식명 row와 `개/g/kcal` 3열 계산값을 확인했습니다.
- 최종검수에서 2026-06-05 14:19 KST Playwright 화면검수 산출물, 로컬 HTTP `200`, 컴파일, 지정 unittest 21개 통과를 확인해 조건부 승인했습니다. 잔여 axe 경고는 별도 접근성 개선으로 분리합니다.

## 2026-06-05 식단 입력 운동 입력 패턴 재구성

- `/app?mode=meal`의 식단 입력을 운동 입력과 같은 `상단 3칸 -> 바로 선택 -> 빌더 -> 저장` 구조로 재구성했습니다.
- `끼니 선택`, `음식 이름`, `입력 기준`을 식단 입력 form 내부 첫 흐름으로 배치하고, 기존 `meal_type` 저장값을 유지했습니다.
- 음식 바로 선택을 `최근`, `즐겨찾기`, `조합` 탭으로 통합하고 음식 빌더 preset 및 대표 음식 입력 동기화를 확인했습니다.
- 최종검수에서 2026-06-05 12:39 KST Playwright 화면검수 재실행, 로컬 HTTP `200`, 컴파일, 지정 unittest 21개 통과를 확인해 조건부 승인했습니다. 잔여 axe 경고는 별도 접근성 개선으로 분리합니다.

## 2026-06-05 식단 구분 선택 노출 회귀 보정

- `/app?mode=meal`에서 `아침`, `점심`, `저녁`, `간식`, `기타` 끼니 선택을 접힌 식단 입력 form 밖의 segmented control로 항상 보이게 보정했습니다.
- 기존 식단 저장 `meal_type` submit 값과 빠른 음식 목록 끼니별 갱신 흐름을 유지했습니다.
- `오늘 연료 상태`가 끼니 선택보다 먼저 보이지 않도록 식단 입력 흐름의 위계를 정리했습니다.
- 최종검수에서 최신 Playwright 화면검수 산출물, 로컬 HTTP `200`, 컴파일, 지정 unittest 21개 통과를 확인해 조건부 승인했습니다. 잔여 axe 경고는 별도 접근성 개선으로 분리합니다.
